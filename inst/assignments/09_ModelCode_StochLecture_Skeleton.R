...
stoch_seir_timestep_closed <- function (state, params,dt) {
  ...
  infection <- rbinom(1, S, exp(-dt*lambda))
  recovery <- rbinom(1, I, exp(-dt*gamma))
  incubation <- rbinom(1, E, exp(-dt*delta))

  return(state + c(-infection, infection - incubation, incubation - recovery, recovery))
}
