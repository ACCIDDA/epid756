...
start <- c(S=999, E=1, I=0, R=0)
params <- c(beta = 2, gamma = 1/5, delta = 1/2, mu = 1/(50*365))

det_dt <- run_seir(60, start_state = start, params = params)
sto_dt <- seq_len(100) |>
    lapply(\(sample) run_stoch_seir(60, start_state = start, params = params, dt = .1)) |>
    rbindlist(idcol = TRUE)

ggplot() + aes(time, count, color = compartment) +
    geom_line(data = det_dt, alpha = 1) +
    geom_line(aes(group = interaction(.id, compartment)), data = sto_dt, alpha = 0.05) +
    theme_bw()
